    ...
    try :
        ...
        if (now.minute % time_unit == 1) & (0 < (now.second % 60) <= 15) :   # N시:01:00초 ~ N시:01:15초 사이 시각이면
            ...
            if bought_state == 0 :   # 매수가 없는 상태라면
                ...
                print ('DF_test_ma[ratio_trend][-1] : {0} greater than (0.9998 * DF_test_ma.iloc[-(ratio_average_duraion_short) : -1, :][ratio_trend].mean()) : {1}.'.format(DF_test_ma['ratio_trend'][-1], (0.9998 * DF_test_ma.iloc[-(ratio_average_duraion_short) : -1, :]['ratio_trend'].mean())))  
